...
import { useOfflineSync } from "@/features/OfflineDataSync/useOfflineSync";
...
export function EmergencyWizard() {
  ...
  const onSubmit = useCallback(async (data: EmergencyFormValues) => {
  ...
  const handleUseCurrentLocation = useCallback(() => {
    setValue("location", "Current GPS Location: 28.59N, 77.34E", { shouldValidate: true });
  }, [setValue]);
      ...
      <div className="bg-[#1C1C1E] border border-white/5 rounded-3xl p-6 shadow-2xl overflow-hidden relative min-h-[400px]">
        <form onSubmit={handleSubmit(onSubmit)} className="h-full">
          <AnimatePresence mode="wait" custom={1}>
            {step === 1 && (
              <motion.div key="step1" custom={1} variants={SLIDE_VARIANTS} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 300, damping: 30 }}>
                <div className="space-y-6">
                  ...
                  <button 
                    type="button" 
                    onClick={() => {
                      if (locationValue.length >= 5) nextStep();
                    }} 
                    ...
                    disabled={locationValue.length < 5}
                  >
                    Next Step
                  </button>
                </div>
            ...
            {step === 2 && (
              <motion.div key="step2" custom={1} variants={SLIDE_VARIANTS} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 300, damping: 30 }}>
                <div className="space-y-6">
                  <div className="flex items-center gap-3 mb-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      const isSelected = emergencyTypeValue === type.id;
            ...
            {step === 3 && (
              <motion.div key="step3" custom={1} variants={SLIDE_VARIANTS} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 300, damping: 30 }}>
                <div className="space-y-6">
                  <div className="flex items-center gap-3 mb-8">
                    <button type="button" onClick={prevStep} className="h-12 w-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mr-2 hover:bg-white/10 transition-colors">
                      <ChevronLeft className="h-6 w-6 text-white" />
